...
| Hoboken construction updates | https://www.hobokennj.gov/hoboken-construction-updates | Current projects, road closures, no-parking areas, closure hours, affected streets, and some corridor-style impacts | Weekly/changing webpage | Medium; no clean API found yet | Nearby construction could be medium; closures, detours, and impacted corridors could be high | Investigating; manual extraction into `construction_impacts.csv` |
...
The current manual rows should always keep the city source URL and a note when coordinates are approximate. Future work is to investigate whether the embedded map is backed by a downloadable GIS layer, ArcGIS service, or another API that would avoid manually drawing the lines.
